...
from formulas import FORMULA_COLS
...
def init_state():
    defaults = dict(
        ...
        all_strikes=[], formula_keys=[], ts_index=0,
        step_size=1, ts_table=None, date_str="",
...
with st.form("params_form"):
    ...
    date_input  = c2.date_input(
        "Session Date", value=date.today())
    ...
    rough_center        = c4.number_input(
        "Rough ATM Strike", value=5580, step=5)
    spot_override_input = c5.number_input(
        "Spot Override (fallback)", value=5580.0, step=1.0)

    submitted = st.form_submit_button(
        "🚀 Fetch & Compute Full Session",
        use_container_width=True)

if submitted:
    if not api_key:
        st.error("Enter your iVolatility API key")
        st.stop()

    # ── Both OI and Greeks use the same session date ──────────
    ...
    # ── Pilot fetch ───────────────────────────────────────────
    with st.spinner("Pilot fetch — establishing price range..."):
    ...
    # ── Step 1 — Option IDs from session date ─────────────────
    with st.spinner("Step 1/3 — Option series..."):
        calls_s, puts_s = get_option_ids(
            api_key, headers,
            date_str,      # ← session date, not prev day
            exp_date_str,
            strike_min, strike_max)
    if calls_s is None: st.stop()
    st.success(
        f"Step 1 ✅  Calls: {len(calls_s)}  Puts: {len(puts_s)}")

    # ── Step 2 — EOD OI from session date ────────────────────
    ...
    st.success(
        f"Step 2 ✅  OI: {filled}/{len(oi_map)} non-zero")
    ...
    with st.spinner("Computing GEX / DEX..."):
        from pipeline import (pivot_wide, prepare_wide,
                              run_full_pipeline,
                              build_minute_series,
                              build_session_table)
        wide_df  = pivot_wide(df_all)
        wide_df  = prepare_wide(wide_df)
        result   = run_full_pipeline(
            wide_df, spot_override_input, date_input)
        ts_table = build_session_table(result)
        ms, sorted_ts, all_strikes, formula_keys = \
            build_minute_series(
                result, spot_override_input, FORMULA_COLS)

    st.session_state.update(dict(
        minute_series=ms, sorted_ts=sorted_ts,
        all_strikes=all_strikes, formula_keys=formula_keys,
        ts_table=ts_table, ts_index=0, computed=True))
    st.success("✅ Done.")
...
if st.session_state["computed"]:
    ...
    with cc3:
        ...
        st.session_state["step_size"] = st.selectbox(
            ...
            index=opts.index(st.session_state["step_size"])
                  if st.session_state["step_size"] in opts
                  else 0,
    ...
    spots    = [v["spot"] for v in ts_data.values()
                if v["spot"] > 0]
    ...
    st.markdown(
        ...
        f"{eat_dt.strftime('%H:%M')} EAT</span>"
        f" &nbsp;|&nbsp; "
        f"<span style='color:#f0c040'>"
        f"Spot: {cur_spot:.2f}</span>"
    ...
    with tc2:
        if chart_type == "GEX":
            ...
            formula_choice = st.radio(
                ...
                index=0, horizontal=True,
                key="dex_formula_radio")

    if not formula_choice:
        formula_choice = (gex_keys[0] if chart_type == "GEX"
                          else dex_keys[0])
    ...
    title      = (f"{fkey}  —  {date_str}  |  "
                  f"{cur_ts} ET  |  Spot {cur_spot:.2f}")

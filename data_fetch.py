...
from config import (BASE_URL, MIN_DELAY, POLL_DELAY,
                    RISK_FREE, SESSION_START, SESSION_END,
                    STRIKE_STEP)
...
def async_download(endpoint, params, headers,
                   label="", max_polls=25):
...
# ─────────────────────────────────────────────────────────────
# STEP 1 — Option IDs
# Uses the session date itself (not previous day) so that OI
# reflects the actual session being analyzed.
# ─────────────────────────────────────────────────────────────
def get_option_ids(api_key, headers, date_str, exp_date_str,
                   strike_min, strike_max):
    """
    date_str     : the session date — OI pulled from THIS date
    exp_date_str : expiration date (same as session for 0DTE)
    """
    dl_url = async_download(
        "/equities/eod/option-series-on-date",
        {"symbol": "SPX", "date": date_str, "apiKey": api_key},
        headers, label="series")
    if not dl_url: return None, None

    series_df = download_csv_gz(dl_url, headers, api_key,
                                label="series")
    if series_df is None or series_df.empty:
        st.error("Empty option series")
        return None, None

    series_df.columns = [c.strip().lower()
                         for c in series_df.columns]
    series_df["expirationdate"] = pd.to_datetime(
        series_df["expirationdate"])
    series_df["strike"] = pd.to_numeric(
        series_df["strike"], errors="coerce")

    exp_mask    = (series_df["expirationdate"]
                   == pd.Timestamp(exp_date_str))
    ...
    filtered = (filtered
                ...
                .drop_duplicates(
                    subset=["strike", cp_col], keep="first")
                .drop(columns=["_is_spx"])
                .reset_index(drop=True))

    return (filtered[filtered[cp_col] == "C"]
            .sort_values("strike"),
            filtered[filtered[cp_col] == "P"]
            .sort_values("strike"))


# ─────────────────────────────────────────────────────────────
# STEP 2 — EOD OI
# Pulls OI from the session date itself.
# iVolatility EOD OI for the session date reflects the OI
# at that session's open — more accurate than prior day.
# ─────────────────────────────────────────────────────────────
def get_eod_oi(api_key, headers, calls, puts, date_str):
    """
    date_str : session date — pulls OI from THIS session
    ...
    def fetch_oi(row, cp_label):
        rec = sync_call(
            ...
            {"optionId": int(row["optionid"]),
             ...
             "apiKey": api_key},
            headers,
            label=f"{cp_label}{int(row['strike'])}")
        if not rec: return
        d = {k.strip().lower().replace(" ", ""): v
             for k, v in rec[0].items()}
        oi_val = (d.get("openinterest")
                  or d.get("open_interest")
                  or d.get("oi")
                  or d.get("openint") or 0)
...
def get_session_price_range(api_key, date_str, exp_date_str,
    ...
    df["underlyingPrice"] = pd.to_numeric(
        df["underlyingPrice"], errors="coerce")
    return (df["underlyingPrice"].min(),
            df["underlyingPrice"].max())
    ...
    for strike in strikes:
        for opt_type in ["C", "P"]:
            ...
            progress_bar.progress(
                count / total,
                text=f"Fetching {strike}{opt_type} "
                     f"({count}/{total})")
            try:
                time_module.sleep(MIN_DELAY)
                df = get_intra(
                ...
                if "optionIv" in df.columns:
                    df["optionIv"] = pd.to_numeric(
                        df["optionIv"], errors="coerce")

...
from constants import BASE_URL, MIN_DELAY, POLL_DELAY, SESSION_START, SESSION_END, STRIKE_STEP, RISK_FREE
from helpers import bsm_vanna, bsm_charm
...
def get_eod_oi(api_key, headers, calls, puts, prev_date_str):
    ...
    def fetch_oi(row, cp_label):
        ...
        oi_val = (d.get("openinterest") or d.get("open_interest") or d.get("oi") or d.get("openint") or 0)
...
# STEP 3a — Pilot fetch
def get_session_price_range(api_key, date_str, exp_date_str, rough_center, minute_type="MINUTE_1"):
    ...
    try:
        df = get_intra(symbol="SPX", date=date_str, expDate=exp_date_str,
                       strike=str(int(rough_center)), optType="C", minuteType=minute_type)
        _last_request_time[0] = time_module.time()
    except Exception as e:
...
def get_intraday_greeks(api_key, date_str, exp_date_str,
                        strike_min, strike_max, minute_type, oi_map, progress_bar):
    import ivolatility as ivol
    from datetime import datetime
    ...
    strikes = list(range(int(strike_min), int(strike_max) + STRIKE_STEP, STRIKE_STEP))
    total = len(strikes) * 2
    all_data = []
    count = 0
        ...
        for opt_type in ["C", "P"]:
            ...
            try:
                ...
                df["_dte_years"] = df["timestamp"].apply(
                    lambda ts: max((close_dt - ts.to_pydatetime()).total_seconds() / (252 * 6.5 * 3600), 1e-8))

                df["optionVanna"] = df.apply(
                    lambda r: bsm_vanna(r.get("underlyingPrice", np.nan),
                                        r["_strike"], r["_dte_years"], RISK_FREE, r.get("optionIv", np.nan)), axis=1)
                df["optionCharm"] = df.apply(
                    lambda r: bsm_charm(r.get("underlyingPrice", np.nan),
                                        r["_strike"], r["_dte_years"], RISK_FREE, r.get("optionIv", np.nan)), axis=1)

...
def build_minute_series(wide_df, spot_override, intra_date):
    ...
    all_strikes = sorted(set(sk for ts_data in series.values() for sk in ts_data.keys()))
...
def build_session_table(wide_df, spot_override, intra_date):
    ...
    ts_df.rename(columns={c: c.replace("_\( ", " ( \))") for c in formula_cols if c in ts_df.columns}, inplace=True)
    return ts_df
